...
fn main() -> Result<()> {
    ...
    let available_sessions = match wasapi_sessions::enumerate_audio_sessions() {
        Ok(sessions) => sessions,
        Err(e) => {
            log::error!("Failed to enumerate audio sessions: {}", e);

            // Print user-friendly error message
            logger::print_error("Failed to access audio devices");
            logger::print_info("");
            logger::print_info("This may be due to missing permissions:");
            logger::print_info("  • macOS: Grant Screen Recording permission in");
            logger::print_info("    System Settings > Privacy & Security > Screen Recording");
            logger::print_info("  • Check that you've accepted the Screen Capture terms");
            logger::print_info("");
            logger::print_info("If the issue persists, try:");
            logger::print_info("  1. Remove this app from Screen Recording permissions");
            logger::print_info("  2. Add it back and restart");
            logger::print_info("  3. If still failing, a system restart may be needed");
            logger::print_info("");
            logger::print_info("Press any key to exit...");
            ...
            match audio::get_input_device_by_name(&host, &session_info.device_name) {
                Ok(device) => {
                    audio::start_input_recording(&device, &mut recording_session, server_config.clone())
                        .map(|stream| (Some(stream), None::<Box<dyn FnOnce() + Send>>))
                }
                Err(e) => {
